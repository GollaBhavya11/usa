...
const Sidenav = ({ role, isSidenavOpen, toggleSidenav, toggleDrawer, isDrawerOpen }) => {
  ...
  const getIcon = (item, isActive) => {
    switch (item) {
      ...
      />; break;
      case 'Expenses': return <Image

        alt="Expenses"
        src={isActive ? expenceAcv : Expenses}
        quality={100}
        width={23}
        height={23}
  ...
  return (<>
    ...
    >
      ...
      <List>
        <div>
          {!token && <Button onClick={(event) => router.push('/')}>Login</Button>}
        </div>
        ...
        {roles[userRole]?.map((item, index) => (
          <div key={index} name={item} className={`p-4 flex items-center gap-4 ${router.pathname === routes[item] && 'text-blue'}`}>
            ...
            <Link href={`${routes[item]}`} onClick={toggleSidenav}>
              {/* <a onClick={toggleSidenav}>{item}</a> */}{item}
            </Link>
          </div>
        ))}
